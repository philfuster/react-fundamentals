...
To style React components use the className prop to add class names and the
style prop to define CSS. The CSS gets defined as an object where the keys are
the CSS properties. The names of those CSS properties change from `kebab-case`
in CSS to `camelCase` in React.

You can create a custom component that encapsulates the styling logic and class
names. This allows the developers to provide sensible defaults for things like
styles and class names. This can make the component easier to use, but still
allow for customization, by allowing the user to provide their own styles and
class names.

Instead of exposing the class names and styles to the user, you can encapsulate
the implementation of the styling by exposing a prop that represents some set of
styling. For example, exposing a `size` prop instead of allowing the user to
provide class name. Then the component can determine the class name and styles
based on the value of the `size` prop.
