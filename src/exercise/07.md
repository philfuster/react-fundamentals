...
When rendering arrays of elements, each element must have a unique `key` prop.
This is because React needs a way to associate the old elements with the new
ones when things change. If you don't provide a `key` prop, React can get things
mixed up. This is because React would have no way of differentiating the
elements between renders. You could have removed an item from the list, added an
item to the list, or changed the values of an existing item on the list. React
performs basic diff, but has no way of guaranteeing it got it right. The
solution is to give each element a unique `key` prop (unique to the array), and
then everything will work fine.

This issue with not providing a `key` prop not only affects the rendering of the
elements, but also the state of the elements, for example the focus state of an
input element, or the selected text. Providing a unique (to the array) `key`
prop fixes these issues.
